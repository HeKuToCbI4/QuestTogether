...
Together Forever's entire purpose is to route it the one way it can travel: voluntarily,
player to player.
...
```lua
QuestTogetherForeverDB = {}          -- SavedVariable; created empty, nothing reads it yet
...
```
## SavedVariables: QuestTogetherForeverDB
```
