...
| B10 | B: disable Quest Together, `/reload`, stay grouped. A: open a quest | A's popup lists B **by name** as `?  (no addon heard from)`. A: `/qt ask Q1` prints, after 3 s, `Quest Q1 -- no answer from:` and the same line for B. | B missing from the list → the roster walk failed; record what `/qt status` shows. |
