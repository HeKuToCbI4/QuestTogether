...
What it does, per test:
  ...
  2. reads QuestTogether.toc and loads every listed file IN THAT ORDER through
...
local function Root()
    ...
    for _, candidate in ipairs({ "", "../", "../../" }) do
        if Exists(candidate .. "QuestTogether.toc") then
            ROOT = candidate
            return ROOT
        end
    end
    error("cannot find QuestTogether.toc -- run the suite from the repository root")
end
...
function M.TocFiles()
    local files = {}
    for entry in io.lines(Root() .. "QuestTogether.toc") do
    ...
    if #files == 0 then error("no .lua files listed in QuestTogether.toc") end
    return files
end
...
function M.newEnv(opts)
    ...
    install("SLASH_QUESTTOGETHER1", nil)
    install("QuestTogetherDB", nil)
    ...
    local ok, err = pcall(function()
        for _, file in ipairs(M.TocFiles()) do
            ...
            chunk("QuestTogether", env.ns)   -- exactly how the client calls it
        end
    end)
    if not ok then
        env.destroy()
