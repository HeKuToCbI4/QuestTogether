...
h.test("an answer from a never-seen sender is recorded (current behaviour)", function(env, ns)
    ...
    h.eq(env.prints[1], "|cff33ff99Quest Together Forever|r Carol has the addon.", "the peer was announced")
end)

h.test("a cross-realm peer is keyed by name and realm", function(_, ns)
    receive(ns, "2|A|" .. QUEST .. "|1", "Carol-Ravencrest")
    local peer = ns.peers[ns.PeerKey("Carol-Ravencrest")]
    h.ok(peer, "keyed by name and realm")
    h.eq(peer.name, "Carol-Ravencrest", "display name")
    h.isNil(ns.peers[ns.PeerKey("Carol")], "not keyed by the bare name")
end)
