...
- A settings page: Esc → Options → AddOns → Quest Together Forever, or `/qtf config`
  (also `/qtf settings`). For now it holds two test checkboxes that do nothing yet.
- **Copy debug info** on that page, and `/qtf debug`: a window with everything the
  addon knows about itself, your client and your group, selected and ready for
  Ctrl+C. Paste it into bug reports.

### Changed
- The slash command is now **`/qtf`** (was `/qt`), to match the addon's name:
  `/qtf ask <id>`, `/qtf ping`, `/qtf status`, `/qtf help` and so on. `/qt` no
...
- The popup now looks like part of the quest frame: a bronze border, the quest's own
  ...
  `/qtf parchment` shows where the parchment came from.
...
- The addon is renamed to **Quest Together Forever**, matching the GitHub repository
  (now `HeKuToCbI4/quest-together-forever-wow-addon`). The folder, the `.toc` filename
  and `## Title` move together (the client silently skips an addon whose folder and
  `.toc` differ), and `SavedVariables` becomes `QuestTogetherForeverDB` (nothing is
  saved yet, so no data is lost). The wire prefix `QTOG` and the `/qt` command are
  unchanged. It was briefly called **Quest With A Friend**, and the v0.0.2 release zip
  ships a `QuestWithAFriend` folder: delete that folder when you install this one,
  or both copies load and every answer is sent twice.

### Fixed
- Every member of your group is now listed, whether or not they run Quest Together Forever.
  ...
  group has Quest Together Forever." instead of the false "(not in a group)". When the roster
