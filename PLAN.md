...
**Document status:** Draft v1 — 2026-09-20
...
| C6 | **Forever inherits Midnight's "secret values" restrictions.** | Quest data is believed unaffected. **Must be verified in M0** — see [R2](#risk-register). |
...
| `Commands.lua` | UI layer (text) | `/qt ask`, `/qt ping`, `/qt status` |
| `Diagnostics.lua` | — | Solo verification commands. **Deletable.** |
| `Core.lua` | Wiring | Bootstrap, event frame, slash dispatch |

Load order is `Compat → Peers → Protocol → Commands → Diagnostics → Core`, as listed
in the `.toc`.
...
- **Channel:** `"PARTY"` when in a party, `"RAID"` when in a raid.
- **Payload:** ASCII, `|`-delimited fields, first field is the protocol revision.
- **Maximum:** keep payloads well under the client's cap (target ≤ 200 chars).
...
|---|---|---|---|
| `HELLO` | broadcast | `1\|HELLO\|<rev>` | Announce presence. Sent on join and in reply to an unknown peer. |
| `LOGS` | broadcast | `1\|LOGS\|<count>\|<packed>` | Full active quest log (delta-packed IDs; **headers excluded**, `questID > 0` only). Sent on change, debounced. |
| `QREQ` | → peers | `1\|QREQ\|<seq>\|<count>\|<packed>` | "Have you completed these?" |
| `QREP` | ← peer | `1\|QREP\|<seq>\|<count>\|<mask>` | Bitmask reply over the request's ID order. |
...
`HELLO` carries the sender's protocol revision. On mismatch:
...
one function (`Cache.applyReply`) and is covered by an explicit unit test.
...
> **Instrumented by v0.1.** The measurement probes that closed items 1, 3, 4 and 5
> (`/qt env`, `/qt probe`) were retired on 2026-09-21 — their results are recorded in
> [§12](#12-open-questions). The one remaining item needs two grouped clients running
> `/qt ping` then `/qt` to confirm item 2.
...
- [ ] Completion oracle wrapping `IsQuestFlaggedCompleted`, local-only.
- [ ] Standalone status panel rendering the local player's state.
...
- [ ] `HELLO` / `LOGS` handling and roster tracking.
...
- [ ] `QREQ` / `QREP` with sequence correlation and 3-second timeouts.
- [ ] Request coalescing and the sliding-window throttle.
- [ ] The tri-state cache, including the "never write `false` from absence" invariant.
- [ ] `/qt status` debug output.
...
- [ ] `pcall` discipline on every inbound message handler — malformed input from a
      peer must never produce a Lua error in *your* session.
...
| R7 | TOC `Interface` number unknown / `16001` inconsistent | Addon won't load | High | Resolve in M0. Ship with `## Interface` verified in-client. |
...
by a forward reference, which any `luac -p` or a linter would not even have needed to
run — simply loading the file in an interpreter would have caught it. Installing Lua
locally is a small, one-time cost against a class of failure we have already hit once.

**Resolved 2026-09-21.** Lua 5.5 is installed at `C:\lua` (`luac55.exe`, `lua55.exe`).
All seven modules pass `luac55 -p`. That is a *parse-only* check — it catches syntax
errors, not the forward-reference/runtime class, so the cross-module "call only at
runtime" rule still carries that weight.
...
- Helper order in `Core.lua` is now deliberate, with a forward declaration and a
  comment explaining why it cannot move.
...
| Q7 | Is `name-realm` a stable peer key? | Open | M2 |
